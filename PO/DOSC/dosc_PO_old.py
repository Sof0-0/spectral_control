...
class DOSC_PO_old(torch.nn.Module):
    def __init__(self, A, B, C,  Q, R, h, H, gamma, eta=0.001, T=100, name="DOSC", nl=False):
        ...
        self.filter_matrix_1, self.filter_matrix_2 = self.construct_filter_matrices()
        ######### FILTERS ##########
    ...
    def construct_filter_matrices(self):
        ...
        2. [1_{H+1}; σ_1^(1/4)φ_1^T; ...; σ_H^(1/4)φ_H^T] ∈ ℝ^{(H+1)×d}
        """
        ...
        # First row is 1_{M}
        filter_matrix_2[0] = torch.ones(self.M, device=self.device) / torch.sqrt(torch.tensor(self.M, dtype=torch.float32, device=self.device))
    ...
    def filter_Y_nat(self, Y_nat):
        ...
        for i in range(self.M):
            intermediate[i] = self.filter_matrix_1 @ Y_nat[i]
        ...
        for i in range(self.h):
            Y_nat_filtered[i] = self.filter_matrix_2 @ intermediate[:, i, :]
