...
class DSC_STU(torch.nn.Module):
    def __init__(self, A, B, C, Q, Q_obs, R, Q_noise, R_noise, h=3, m=None, T=100, lr=0.01, name="DSC", nl=False, gamma=0.1):
        ...
        self.M_stu = torch.nn.Parameter(torch.ones(20, self.m_control, self.d) * 0.009)
        ...
        self.M_bar = torch.nn.Parameter(
            torch.ones(self.m_control, self.p, device=self.device) * 0.009
        )
        ...
        self.M = torch.nn.Parameter(
            torch.ones(self.h+1, num_eigenvalues, self.m_control, self.p, device=self.device) * 0.009 # 0.045 for linear
        )
